"""
Rottura Model - Gestione File Rotture Excel
"""
...
class Rottura(db.Model):
    ...
    Attributes:
        id: ID file rotture (primary key)
    ...
    @staticmethod
    def get_by_anno(anno: int) -> list['Rottura']:
        """
        Recupera tutti i file rotture per un anno specifico

        Args:
            anno: Anno di riferimento

        Returns:
            Lista di oggetti Rottura
        """
        return Rottura.query.filter_by(anno=anno).order_by(Rottura.created_at.desc()).all()

    @staticmethod
    def get_pending() -> list['Rottura']:
        """
        Recupera tutti i file in attesa di elaborazione

        Returns:
            Lista di oggetti Rottura con esito 'Da processare'
        """
        return Rottura.query.filter_by(esito='Da processare').order_by(Rottura.created_at).all()

    def to_dict(self) -> dict:
        """
        Converte l'oggetto Rottura in dizionario

        Returns:
            Dizionario con dati rottura
        """
        return {
            'id': self.id,
            'anno': self.anno,
